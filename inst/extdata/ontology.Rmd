...
+ *dea_factsheets.rda* (N = `r nrow(dea_factsheets)` records) has `r length(category)` drug categories and `r nrow(class)` classes (e.g., `r class$class[1]`, `r class$class[2]`, etc.)
...
+ *slang.rda* (N = `r nrow(dea_street_names)` records) contains `r length(unique(dea_street_names$class))` drug classes and `r length(unique(dea_street_names$brand)) - 1` brands (e.g., `r unique(dea_street_names$brand)[2:4]`). 
